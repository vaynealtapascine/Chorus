...
- [~] M8.4 Chat notifications (mentions, DMs, replies), inline reply — cross-account mentions/replies/DMs queued on ingest, inbox + push (activity.rs); per-channel level (all/mentions/none) + per-kind switches as account prefs, web controls; own-account internal activity, per-member mention rules and Android inline reply not yet
