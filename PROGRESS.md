...
- **In progress:** batch 2 is assigned to gpt-6-sol on branch `sol/batch-2` (worktree `F:\DunBuild\Chorus-sol`, task list + log in `docs/handoff/sol-batch-2.md` on that branch: T1 blobs … T13 hardening). Claude audits + merges it later. Don't start those tasks on `main`.
- **Owner:** —
