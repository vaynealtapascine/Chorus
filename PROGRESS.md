...
- **In progress:** three in parallel, toward v1 (owner, 2026-09-24: one full stable build working end to end on phone, web and desktop — the PWA usable with the server down, done — then a GitHub Pages landing page; D-068). **Claude (local)**: audits/merges, the end-to-end v1 run on the owner's server, Web Push, deploys, landing page. **gpt-6-sol**: Android parity, `docs/handoff/sol-batch-5.md` (V1–V7) on `sol/batch-2`. **Remote Claude** (higher limits, so the long tasks): CI, M5.10 channel permissions, post search, shareable feeds, M7 REST reads, a Playwright e2e suite, the export bundle, `docs/handoff/opus-remote-2.md` (R8–R15) on `handoff/opus-remote-1`. Each file has a don't-touch list.
