...
- **In progress:** (owner, 2026-09-25) no release until the owner's UI redesign lands (Figma, from docs/FLOWS.md + ENTITIES.md); until then features + a robust core and messaging/channels. **Remote Claude**: batch R5 (`docs/handoff/opus-remote-5.md`): R35 (flaky rest_fuzz, fixed on the server) merged; R30–R34 (PluralSpec implementation) paused by the owner while the spec is still being worked on; the owner asked the remote for modular, out-of-the-way work instead. **Claude (local)**: op payload validation hardening + fuzzing, audits/merges. **gpt-6-sol**: Android (`docs/handoff/sol-batch-5.md`). Chorus Home paused until the designs.
...
- [~] M14.4 Owner decisions: D0–D4, D6, D7, D12, D14, D15 settled 2026-09-26 and applied (name PluralSpec, PluralPort superset, UTF-16 default offsets, CC BY-NC-SA 4.0); D8 (visibility classes with permission grants), D9 (age) and D14 (schema CC BY 4.0, text CC BY-NC-SA 4.0) settled later the same day; D5 settled (subsystem internal fronts, composite members — collectives, blends, fusions — blend ids with an optional degree, partial decoherence, fronting on behalf of a subsystem); D11 (history events) settled 2026-09-27; D10 (enums vs vocabularies) and D13 (chat scope) remain proposed
...
- [-] M14.9 R34 Chorus export/import + round-trip test — remote R5; M14.5–M14.9 paused by the owner (2026-09-26) until the spec is pinned down
...
- 2026-09-25 claude-opus-5.5 — Validation hardening (0e22431): per-op savepoint at ingest (an unprojectable op is refused alone, `unprocessable`, never the batch); `op::FIELD_RULES` + per-kind value rules in core, checked against the SQL schema by a test; fuzz tests for ingest (20k random batches clean) and the client projector. Fixed what they found: a rebuild dropped members whose role had changed (nothing sends set_role yet, so no real data was affected), draft.set couldn't be stored, created_at/owner taken from the earliest op, space.set_roles without roles broke rebuilds.
- 2026-09-25 claude-opus-5.5 — Merged gpt-6-sol's 41 Android commits (a2033cd): V0 Changes.removed persisted, V4 journal/profiles/people (posts, replies, reactions, lists, feeds, highlights, relationships, follower history/stats, Advanced sharing), V5 settings + offline/server search, V3 attachment opening, V6 chat stage with saved plans, widget pins + pinned shortcuts. verify --quick --android passes; all compiled + unit-tested, device audits pending an owner-approved phone session. Also today: cross-space op references and foreign-member authorship refused at ingest (df3dde1, 38e4288); entity/segment ranges validated (3518d97).
- 2026-09-25 claude-opus-5.5 — Deployed to the PC (owner, 19:23; migrations 0007/0008 behind a pre-migrate backup; Android 0.1.380). deploy.ps1 now builds on F: (the first attempt stopped on the full C: drive). More ingest ownership rules: only a message's author edits it (D-073), attachments and existing message ids stay their creator's (c2a7adb); validation split so value rules apply to new ops only (aafdd86). Fuzz clean after all of it.
- 2026-09-25 gpt-6-sol — Owner-approved Galaxy A56 audit of `sol/batch-2`: installed in place, checked Stage selection/capture, light redaction, status bar restoration, Search → Chat, file chooser and the existing widget's readout. Fixed light capture's invisible system icons and the wrapped search-match Dismiss action; full quick verifier passed. Wider Android device audit remains (details in `docs/handoff/sol-batch-2.md`).
- 2026-09-25 gpt-6-sol — Merged `main` at 9bf6fa0 into `sol/batch-2` for stricter op validation and server ownership rules. Resolved only the progress-log conflict, rebuilt Android FFI and web wasm, and passed the full quick Android verifier. Merged code awaits its phone audit (details in `docs/handoff/sol-batch-2.md`).
- 2026-09-25 gpt-6-sol — V8 Android offline-file foundation: verified, account-scoped durable blob copies; catalogue extraction for live attachments, avatars and emoji; bounded three-file fill. Android unit/build and full quick Android verifier pass. Setting, automatic fill and manual recheck remain (details in `docs/handoff/sol-batch-2.md`).
- 2026-09-25 gpt-6-sol — V8 Android device setting and full-sync controls implemented: delayed file fill, scope recheck with repair progress, file progress and space used in Settings. Full quick Android verifier passes; phone audit batched for a later session (details in `docs/handoff/sol-batch-2.md`).
- 2026-09-25 gpt-6-sol — V8 follow-up: retained files can be opened through Android's file provider; concurrent reads use separate partial downloads. Full quick Android verifier passes; phone audit remains batched (details in `docs/handoff/sol-batch-2.md`).
- 2026-09-25 claude-opus-5.5 — Merged remote batch R3 (0a92e0a): SYNC §9.3 chaos test against the real binary (passes on Windows too), simulator with spaces/channels/scope loss, SPEC §5 chat semantics (edit history, search filters, @group/@account/@front, default speaker D-074, reply elsewhere/privately, per-member reading, slow mode = Q17 default, space roles, Sync issues), windowed replica for tabs (D-075), chat-path budgets. Its fixes: files lost by a restore, a stalled web upload queue, scope-loss copies, read marks leaking to other accounts. Merged gpt-6-sol V6 (f8e9c64): stage layouts/themes, per-widget scope, first owner-approved phone audit. Workspace tests, web check/vitest/e2e (15) and verify --android all pass. wasm is at 288/300 KB gz: next core work must watch it. Sol's list gained V10 (restore file re-upload first).
- 2026-09-25 gpt-6-sol — Merged `main` through 94b84f2 into `sol/batch-2` to take R3 core/FFI and the new V10 handoff. Preserved both progress logs; Android FFI and web wasm rebuilt, and full quick Android verifier passed. No phone run (details in `docs/handoff/sol-batch-2.md`).
- 2026-09-25 gpt-6-sol — V10a Android restore-file recovery implemented: local verified blobs queue durably after a reconcile welcome and upload independently of normal files, accepting HEAD 200/403. Android build, JVM tests and full quick Android verifier pass; phone audit pending (details in `docs/handoff/sol-batch-2.md`).
- 2026-09-25 gpt-6-sol — V10b Android Sync issues: refused ops persist and appear in Settings with reason, copyable text and Dismiss. Android build, unit tests and full quick Android verifier pass; batched phone audit pending (details in `docs/handoff/sol-batch-2.md`).
- 2026-09-25 gpt-6-sol — V10c Android chat edit history: edited marker from the projection, local core revisions on demand, and per-version CW reveal. Android build, JVM projection test and full quick Android verifier pass; phone audit pending (details in `docs/handoff/sol-batch-2.md`).
- 2026-09-25 gpt-6-sol — V10c Android channel speaker defaults: account pref modes plus a core default-speaker adapter, including latch outside the latest chat window; Advanced controls in Chat. Android build, JVM tests and full quick Android verifier pass; phone audit pending (details in `docs/handoff/sol-batch-2.md`).
- 2026-09-26 gpt-6-sol — V10c Android message search now uses the core parser/filter and sends the same raw syntax, including filter-only queries, to server paging. Android build, JVM tests and full quick Android verifier pass; no phone run (details in `docs/handoff/sol-batch-2.md`).
- 2026-09-26 gpt-6-sol — V10c Android per-member reading: Advanced pref, core-selected readers and unseen dots from account-local marks. Android build, JVM tests and full quick Android verifier pass; no phone run (details in `docs/handoff/sol-batch-2.md`).
- 2026-09-26 gpt-6-sol — V10c Android Reply privately opens a member DM or account DM and carries the original ID into the destination composer. Android build, JVM tests and full quick Android verifier pass; no phone run (details in `docs/handoff/sol-batch-2.md`).
- 2026-09-26 gpt-6-sol — Android Insights uses the local front projection and shared core daily totals, with four read-only charts, data tables and CSV saving. Android build, JVM tests and full quick Android verifier pass; phone audit remains batched (details in `docs/handoff/sol-batch-2.md`).
- 2026-09-26 gpt-6-sol — Person accounts now land in Android Journal, can open their own Profile, and no longer see system-only front metrics there. Android build and full quick Android verifier pass; phone audit pending (details in `docs/handoff/sol-batch-2.md`).
- 2026-09-26 gpt-6-sol — Android accepts shared text and files from other apps, lets the user choose a post or channel, and opens a reviewable draft. Android build and full quick Android verifier pass; phone audit batched for later (details in `docs/handoff/sol-batch-2.md`).
- 2026-09-26 gpt-6-sol — Added Android launcher shortcuts for Switch out and New entry beside Switch; the first queues an undoable empty-front switch, the second opens the Journal entry editor. Android build and full quick Android verifier pass; phone audit batched for later (details in `docs/handoff/sol-batch-2.md`).
- 2026-09-26 gpt-6-sol — Android's quick-switch search now excludes foreign members from shared spaces, matching Home and the pinned shortcut's account guard. Android build and full quick Android verifier pass; phone audit pending (details in `docs/handoff/sol-batch-2.md`).
...
- 2026-09-26 claude-opus-5.5 — M14 PluralSpec: permission grants on visibility classes (SPEC §4.8), licence split (schema/examples CC BY 4.0, text CC BY-NC-SA 4.0), D8/D9/D14 settled; D5 composite-identity proposal + questions in DESIGN_NOTES; R5 hand-off updated
- 2026-09-26 claude-opus-5.5 — M14 PluralSpec D5 after the owner's answers: composite members (COLLECTIVE/BLEND/FUSION), subsystem collective_member_id, span part_member_ids / blend_id / for_group_id, invariants 6-8, authorship unchanged (several authors); example and R5 hand-off updated
- 2026-09-26 claude-opus-5.5 — M14 PluralSpec D5 settled: optional blend_degree (0-1) on spans and switch entries, invariant 9; composite parts are never implied authors
- 2026-09-26 claude-opus-5.5 — merged remote R5 (6560d11): R35 `drain.rs` drains unread request bodies so early answers (401/404/413/415/429) no longer reset connections on Windows; `rest_fuzz` + `early_answer` passed 5/5 on the PC (was ~50% failing); verify.py prints UTF-8 when piped. R30–R34 paused by the owner
- 2026-09-27 claude-opus-5.5 — M14 PluralSpec D11 (history events) settled by the owner
