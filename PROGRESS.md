...
- **In progress:** three in parallel, toward v1 (owner, 2026-09-24: one full stable build working end to end on phone, web and desktop PWA; then a GitHub Pages landing page). **Claude**: M5.10 channel permissions, then the end-to-end v1 run on the owner's server, Web Push, landing page. **gpt-6-sol**: Android parity, `docs/handoff/sol-batch-5.md` (V1–V7) on `sol/batch-2`. **Remote Claude**: CI, post search, shareable feeds, M7 REST reads, a Playwright e2e suite, `docs/handoff/opus-remote-2.md` (R8–R14) on `handoff/opus-remote-1`. Each file has a don't-touch list keeping the three apart.
