<script lang="ts">
  ...
  async function load() {
    ...
    error = '';
    try {
      const response = await fetch(`${apiBase()}/posts/${encodeURIComponent(postId)}?depth=1`, {
      ...
      replies = ((await response.json()) as { replies?: Reply[] }).replies ?? [];
    } catch (cause) {
...
<style>
  ...
  .refresh, .muted, .by { color: var(--ink-3); font-size: var(--fs-sm); }
