...
| D-062 | **Linux / public VPS deployment** alongside the Windows PC one (owner request 2026-09-24):<br>• `scripts/pack-linux.ps1` builds a static x86_64 musl `chorus-server` in the `rust:1.98` image (Docker under WSL), bundles the web app, Android update files and `deploy/linux/install.sh`, optionally with a verified data snapshot (`-WithData`).<br>• `install.sh` (Debian 12+/Ubuntu 22.04+): a `chorus` service user, `/opt/chorus/{app,data,backups}`, `/etc/chorus/chorus.toml`, a sandboxed systemd unit, Caddy with the site in `/etc/caddy/sites/chorus.caddy` (imported, not a rewritten Caddyfile), imports through the normal restore path (epoch bump → devices re-send).<br>• Being public changes the threat model: `security.webhook_targets` (`internal` default with loopback now refused, `public` on the VPS, `any`); checked addresses are pinned and redirects not followed; the server stops cleanly on SIGTERM. | claude-opus-5.5 |
