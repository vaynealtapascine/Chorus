...
#### V4 · Local member front and message stats (2026-09-25)
- **Status: code complete, device audit pending.** The Android model now retains own-account front intervals, the configured system timezone and counts across all locally visible messages, not just the latest 100 rendered in each channel. The member profile shows front hours for the last 7 and 28 local calendar days, last fronted, and message count. The calendar windows use the configured zone so daylight saving transitions are handled by their actual elapsed duration. These are own-account stats only; follower stats remain the server's revealed, rounded numbers. No core or server semantics changed.
- **Verified by running:** Android `:app:testDebugUnitTest :app:assembleDebug --offline` and `python scripts/verify.py --quick --android --offline` passed. JVM tests cover projection extraction and a spring-forward transition with a span crossing a seven-day boundary.
- **Compiled/unit-tested only:** No phone UI was run. Later compare one member's 7/28-day totals against the web on an account with switch history and check a co-front interval. Local custom field values remain to be shown offline.

#### V4 · Offline custom fields on own member profiles (2026-09-25)
- **Status: code complete, device audit pending.** The Android projection now supplies live custom field definitions and per-member values in definition order. Own member profiles show text, arrays and boolean values from the local replica, so the fields remain visible without the profile API. Deleted definitions, absent rows and null values stay hidden. Online post counts, relationships and outside highlights continue to use the server-filtered profile bundle. No core or server semantics changed.
- **Verified by running:** Android `:app:testDebugUnitTest :app:assembleDebug --offline` and `python scripts/verify.py --quick --android --offline` passed. A JVM projection test checks ordering, typed display, deletion, missing values and isolation between members.
- **Compiled/unit-tested only:** No phone UI was run. Later edit a field on a disposable member offline, restart and compare the profile with web. V4 profile media and relationship curation remain; V5 settings/search is the next broad Android gap.

#### V4 · Offline profile relationships and editing (2026-09-25)
- **Status: code complete, device audit pending.** The own-member Relationships tab now reads live relationship types and links from the local projection, including incoming inverse names and external labels. It can create private types and links, remove links, and delete unused types through queued account ops while offline. Deleted or malformed local rows are hidden. The online profile bundle is still used for outside highlights and post counts, but relationships no longer require that read. No core or server semantics changed.
- **Verified by running:** Android `:app:testDebugUnitTest :app:assembleDebug --offline` and `python scripts/verify.py --quick --android --offline` passed. JVM tests check live/deleted projection rows, outgoing and incoming labels, external targets, and private create payloads.
- **Compiled/unit-tested only:** No handset UI was run. Later create a reciprocal and an external relationship on a disposable account, sync, delete one offline, and compare with web. Profile media and V5 settings/search remain.

#### V5 · Android account settings entry point and preference controls (2026-09-25)
- **Status: partial settings, device audit pending.** The app header opens a Settings screen without crowding the six primary tabs. It reads independent account `pref` rows with pending-local precedence and the legacy account-settings fallback for the default follower ceiling (D-063). Basic controls write mention, DM and reply notification switches, plus the system's default follower preset. Advanced reveals own-switch pings, reply-as-mentioned, history/stats sharing, CW expansion and speaker-annotation parsing. Each action queues one `pref.set` with `device: ''` and preserves the other keys in object-valued prefs. No core or server semantics changed.
- **Verified by running:** Android `:app:testDebugUnitTest :app:assembleDebug --offline` and `python scripts/verify.py --quick --android --offline` passed. JVM tests check pending/stamped precedence, legacy fallback, defaults, preservation of unrelated notification kinds and the exact account-pref payload.
- **Compiled/unit-tested only:** No handset UI was run. Later check narrow header layout, toggle one key offline and after sync, and compare with web. Quiet hours (a follow API setting), appearance/terminology/device/data controls, and search remain outside this slice.

#### V5 · Quiet hours for followed accounts in Android Settings (2026-09-25)
- **Status: code complete, device audit pending.** Settings now loads the signed-in account's follow list, shows its current quiet window, and updates or clears that window for every active follow through the existing `PUT /follows/{id}/prefs` route. It validates 24-hour local times, sends the current UTC offset, preserves each follow's unrelated prefs, and reports when the server read is unavailable. These follow preferences are server-owned; the change requires a connection. No core or server semantics changed.
- **Verified by running:** Android `:app:testDebugUnitTest :app:assembleDebug --offline` and `python scripts/verify.py --quick --android --offline` passed. JVM tests check follow-pref parsing, local time validation, offset and unrelated-pref preservation.
- **Compiled/unit-tested only:** No phone UI or live follow request was run. Later update quiet hours with two disposable active follows and check both retained any other prefs. V5 search and the rest of settings remain.

#### V5 · Local Android search corpus and prefix index (2026-09-25)
- **Status: data layer complete, UI/server paging next.** The typed model now keeps all locally visible messages as search documents, including messages older than the chat screen's latest 100 per channel. An on-demand index searches messages, own journal posts and unretracted switches by folded word prefixes, with `from:`, `in:`, `has:`, `before:` and `after:` filters aligned with the web's local search where applicable. It builds once when opened and keeps newest-first order ready for fast queries. The search UI and older server pages are still to be connected. No core or server semantics changed.
- **Verified by running:** Android `:app:testDebugUnitTest :app:assembleDebug --offline` and `python scripts/verify.py --quick --android --offline` passed. JVM tests cover cross-kind matching/filtering, the >100-message local case, and a 5,000-post/10,000-switch query budget under 16 ms average on this machine.
- **Compiled/unit-tested only:** No handset UI was run. Wire an in-app Search screen to this corpus, add the server-filtered `GET /search/messages` cursor for older hits, and audit with a large disposable replica.

#### V5 · Android offline Search screen (2026-09-25)
- **Status: local UI complete, server paging next.** A header search control now opens a full-screen Search view with Messages, Posts and Switches tabs. The prefix index is built off the UI thread when the screen opens; local result cards show channel/author and time, keep CW bodies and titles collapsed until revealed, and return to the previous primary tab on close or Back. Settings also now closes on Back. Search includes all locally visible messages, not just those currently rendered in chat. No core or server semantics changed.
- **Verified by running:** Android `:app:testDebugUnitTest :app:assembleDebug --offline` and `python scripts/verify.py --quick --android --offline` passed, including the local corpus and query tests from the preceding slice.
- **Compiled/unit-tested only:** No phone UI was run. Audit keyboard, tabs and CW collapse on a handset; add server-filtered message pages with cursor and a navigation route to a selected result.

#### V5 · Server-filtered older message search pages (2026-09-25)
- **Status: code complete, device audit pending.** Search now sends text terms and supported filters to `GET /search/messages` when live, debounces the first request, appends cursor pages on demand and removes IDs already present in the local result set. The signed-in server's visibility check governs each remote page. Remote cards reuse CW collapse, and their in-memory results are cleared when the query, session, online state or local model changes; local matches remain available offline. Empty-term filter queries stay local because the server requires a nonempty FTS `q`. No core or server semantics changed.
- **Verified by running:** Android `:app:testDebugUnitTest :app:assembleDebug --offline` and `python scripts/verify.py --quick --android --offline` passed. JVM tests check URL encoding of filters/cursor and parsing of server rows and warnings; the full suite ran the server's search visibility test.
- **Compiled/unit-tested only:** No handset or live search request was run. Later search an older message and a message in a revoked space from a disposable account, page through the cursor, and check the CW body remains hidden until reveal. Foreign post search and result navigation remain.

#### V5 · Server-filtered post search pages (2026-09-25)
- **Status: code complete, device audit pending.** The Posts tab now adds `GET /search/posts` pages when live, so readable posts from followed accounts appear beside own offline results. It sends text and date filters, pages with the opaque cursor, de-duplicates local IDs, and uses the server-returned author cards for labels and `from:` filtering. CW title/body stays collapsed in every result card. The server's current audience rules decide each remote page; stale in-memory remote cards clear on query, session, online or model changes. No core or server semantics changed.
- **Verified by running:** Android `:app:testDebugUnitTest :app:assembleDebug --offline` and `python scripts/verify.py --quick --android --offline` passed. JVM tests check URL/cursor encoding, foreign author IDs/names, and CW parsing; the full suite ran the server's post search audience test.
- **Compiled/unit-tested only:** No phone or live cross-account search request was run. Later search a disposable follower-only post before and after unfollowing, page results, and inspect CW reveal. Result navigation remains to be added.

#### V5 · Open search results in their context (2026-09-25)
- **Status: code complete, device audit pending.** Post results now open Journal's existing reply thread, and message results with a locally known channel open that channel in Chat. A compact search-match card keeps an older message visible above the latest-100 chat window; it preserves CW collapse and can be dismissed. Search and Settings navigation close on Back, and account changes clear any held search-match preview. No core or server semantics changed.
- **Verified by running:** Android `:app:testDebugUnitTest :app:assembleDebug --offline` and `python scripts/verify.py --quick --android --offline` passed after correcting a missing Compose import.
- **Compiled/unit-tested only:** No handset navigation was run. Later open an older server hit, confirm its channel and warning, then open a foreign post thread and return with Back. A server-only message in a channel absent from the local replica is still a read-only search card.

#### V4 · Own-member profile Media tab (2026-09-25)
- **Status: readout complete, device audit pending.** Own post rows now resolve attachment IDs through locally visible attachment rows in order, skipping missing rows or hashes. Member profiles list those files in a Media tab, with post-level CW collapse plus each attachment's spoiler reveal and image preview through the existing blob cache. File names and alt text remain available even when bytes are offline. This is read-only media; post attachment composition and general file opening belong to later V3 work. No core or server semantics changed.
- **Verified by running:** Android `:app:testDebugUnitTest :app:assembleDebug --offline` and `python scripts/verify.py --quick --android --offline` passed. A JVM projection test checks attachment ordering, missing/deleted rows, thumbnail hash, alt text and spoiler flag.
- **Compiled/unit-tested only:** No handset media UI was run. Later open a disposable entry with two images and a file, test CW/spoiler reveal, then disable network and inspect cached preview. File opening and post attachment uploads remain.

#### V4 · Attachments in Android journal post cards (2026-09-25)
- **Status: readout complete, device audit pending.** Own Journal cards, lists and feeds, and shared posts in People and member profiles now show attachment metadata and image previews through the existing authenticated blob cache. A post CW gates its attachments, and each attachment keeps its separate spoiler reveal. The shared-post parser accepts only entries with both an ID and blob hash; the server-filtered post endpoint continues to decide which foreign posts are readable. No core or server semantics changed.
- **Verified by running:** Android `:app:testDebugUnitTest :app:assembleDebug --offline` and `python scripts/verify.py --quick --android --offline` passed. A JVM parser test checks shared attachment metadata, spoiler and malformed entry filtering.
- **Compiled/unit-tested only:** No handset UI was run. Later inspect an own and followed CW post with image and file attachments; verify image retrieval and both reveal layers. Post attachment composition and general file opening remain.

#### V3 · Open Android attachments in another app (2026-09-25)
- **Status: code complete, device audit pending.** An attachment card now offers Open file after its post CW and attachment spoiler gates. The original blob is read from the pending queue or chat cache, or downloaded through the authenticated blob client, then shared through a read-only `FileProvider` URI and Android's chooser. The provider exposes only the chat cache and pending blob directories. Opening reports unavailable bytes or a missing handler in the card. No core or server semantics changed.
...
- **Compiled/unit-tested only:** The phone was off adb. Later open a disposable PDF and image in another app, then repeat offline with a cached file and a missing file. Post attachment uploads and a broader media gallery remain.

#### V3/V4 · Compose Android journal posts with attachments (2026-09-25)
- **Status: code complete, device audit pending.** The Journal composer can pick multiple images or files, edit image alt text and mark each file as a spoiler. Posting stages each original and image thumbnail under its hash, queues account-scoped `post.attachment` ops, references their IDs in `post.create`, then schedules the existing resumable upload worker. A failed post keeps the composer and selected files for retry. The same local image cache and CW/spoiler readout from earlier slices displays the resulting post offline. No core or server semantics changed.
- **Verified by running:** Android `:app:testDebugUnitTest :app:assembleDebug --offline` and `python scripts/verify.py --quick --android --offline` passed. A JVM post-payload test checks attachment ID order.
- **Compiled/unit-tested only:** The phone was off adb. Later attach two disposable files, disconnect, post offline, restart, reconnect and check the original/thumbnail bytes, metadata and follower access. A broad media gallery remains.
- **Build environment note:** C: reached 0 bytes free during this slice. The first full verifier failed in Rust temp-file tests; with `TEMP`, `TMP` and `TMPDIR` on `F:\DunBuild\temp-sol`, Rust passed but npm could not write its C: cache. Setting `npm_config_cache=F:\DunBuild\npm-cache-sol` as well let the entire verifier pass. Continue routing those locations to F: until C: has space again; no C: cleanup was attempted from this worktree.

#### V6 · Android chat Stage selection and capture (2026-09-25)
- **Status: partial Stage, device audit pending.** Every local channel now opens a Stage view over the messages already visible in that reader's Chat view. The shared `chorus_core::stage` planner supplies picked rows, folded context, reply-bar visibility, stable redacted/fake names and shifted/hidden times; editing keeps unpicked rows available to select. Capture renders the selected plan at phone width while hiding the app header, bottom navigation and Stage controls; a Done pill fades after two seconds and Android Back exits. CW text stays collapsed until explicitly revealed, and attachments show only a count. Stage changes never write message data. Saved stages, richer styles, attachment rendering/blur and additional filters remain.
- **Verified by running:** Android `:app:testDebugUnitTest :app:assembleDebug --offline` and `python scripts/verify.py --quick --android --offline` passed. A JVM adapter test checks the exact JSON sent to core and the returned row/name parse; Rust core Stage tests ran in the full suite.
- **Compiled/unit-tested only:** No handset run. Later select two messages, switch Fold/Hide/Show, capture and verify that header/nav disappear and fake names/times do not affect Chat. Check that the Done pill can be brought back by tapping. The Stage route currently uses the newest 100 local messages per channel.

#### V6 · Save and reopen Android chat Stages (2026-09-25)
- **Status: code complete for the supported Stage subset, device audit pending.** Save writes an account-scoped `stage.save` definition with channel ID, selection, unselected mode, redacted/fake names and hidden/shifted times. Live definitions appear from the local projection and can be loaded or tombstoned offline with `stage.delete`. A web Stage that uses a style, theme, width, filter, color or time mode Android does not yet render is labelled as a web view and cannot be loaded; this avoids silently changing its meaning. No core or server semantics changed.
- **Verified by running:** Android `:app:testDebugUnitTest :app:assembleDebug --offline` and `python scripts/verify.py --quick --android --offline` passed. JVM tests cover definition serialization, compatible/incompatible load and live/deleted projection rows.
- **Compiled/unit-tested only:** No handset run. Later save a view offline, restart, load it, sync it to web and delete it. The Stage route still covers only the newest 100 locally displayed messages per channel; richer Stage controls remain.

#### V6 · Manual widget member pins (2026-09-25)
- **Status: code complete, device audit pending.** Home now has an Arrange widget pins mode. Tapping a member tile pins/unpins it, and the pinned list can move each member up or down. The root widget grid reads this device's per-account order and puts live pinned members before recents without duplicates; archived pins remain stored but hidden until the member is active again. A pin edit refreshes all widget instances. Folder ordering stays as specified. No core or server semantics changed.
- **Verified by running:** Android `:app:testDebugUnitTest :app:assembleDebug --offline` and `python scripts/verify.py --quick --android --offline` passed. A JVM tile-order test covers duplicates, recent overlap and archived members.
- **Compiled/unit-tested only:** The phone was off adb. Later pin/reorder two disposable members, inspect a 2×1 and expanded widget, restart, and switch accounts to confirm separate pin lists. Dynamic pinned shortcuts and per-widget scope remain.

#### V6 · Launcher shortcuts for pinned members (2026-09-25)
- **Status: code complete, device audit pending.** The top four live pins now publish dynamic Android launcher shortcuts with member glyph icons. Pin edits and relevant model updates refresh them. A shortcut enters the existing search launcher, checks its saved account ID against the current signed-in account and checks that the member is still active and owned by that account, then applies a Replace switch and closes. An old shortcut gives a short unavailable message. Home/widget pin candidates also exclude foreign members in shared spaces. No core or server semantics changed.
- **Verified by running:** Android `:app:testDebugUnitTest :app:assembleDebug --offline` and `python scripts/verify.py --quick --android --offline` passed. JVM tests cover top-four selection, duplicate/archived filtering and foreign-member exclusion.
- **Compiled/unit-tested only:** The phone was off adb. Later pin five disposable members and inspect launcher ordering, tap one, then switch accounts and confirm an old shortcut refuses to switch. Per-widget scope remains.

#### V6 · Stage member and reply filters (2026-09-25)
...
#### V6 · Stage channel heading and reply-bar controls (2026-09-25)
- **Status: code complete, device audit pending.** Stage now includes the channel heading inside the capture area by default. Advanced can hide that heading or hide reply bars; both options survive Save/Load and web-authored views using these render flags can load on Android. Reply visibility still comes from the shared core plan. No core or server semantics changed.
- **Verified by running:** `python scripts/verify.py --quick --android --offline` passed. A JVM definition test checks both flags and their round-trip.
- **Compiled/unit-tested only:** No phone capture was inspected. Later capture a disposable channel with a reply, with each option on and off. Other Stage styles and older-message paging remain.

#### V6 · Transcript and Minimal Stage layouts (2026-09-25)
- **Status: code complete for these layouts, device audit pending.** Android Stage Advanced now offers Chorus, Transcript and Minimal. Transcript places the author with the text and omits times; Minimal omits the author and time row. Both values round-trip through saved definitions, including web-authored stages. Folded context rows use their list position as a unique key, avoiding collisions when several gaps have the same count. Discord-ish, Bubbles and Card still open as web views on Android. No core or server semantics changed.
- **Verified by running:** `python scripts/verify.py --quick --android --offline` passed. JVM definitions check both new style values and continue rejecting Discord-ish.
- **Compiled/unit-tested only:** No phone capture was inspected. Later capture a disposable conversation with a CW, reply and attachments in each layout, and check editing selection markers. Other Stage styles and older-message paging remain.

#### V6 · Page older locally replicated messages into Stage (2026-09-25)
- **Status: code complete for local history, device audit pending.** Stage starts with the same newest 100 messages as Chat and can load 100 older visible messages at a time from the existing projection. The store-thread read filters the account/member view before applying the page limit, so the older-row control does not reveal hidden rows. Chat's regular 100-message model stays bounded. A channel/account change invalidates the Stage window before it can render. No sync, server or core semantics changed.
- **Verified by running:** `python scripts/verify.py --quick --android --offline` passed. A JVM model test checks first window, expansion, ordering and visibility before counting older rows.
- **Compiled/unit-tested only:** No phone scroll was inspected. Later test a channel with over 100 disposable messages and a member-only aside. Server-only history outside the local replica still needs a separate read path.

#### V6 · Stage avatars and conceal control (2026-09-25)
...
#### V8 · Durable Android offline-file foundation (2026-09-25)
- **Status: foundation only.** `Blobs.keep` promotes a blob from the pending queue or evictable caches (or fetches it) into `filesDir/kept-blobs/<account>`, verifying its SHA-256 and 20 MB limit before the copy becomes visible. `Blobs.file` checks this account's durable copy before the cache. Pending uploads remain separate. `OfflineFiles.filesOf` collects deduplicated hashes from live attachment thumbnails/originals, member and group avatars, and custom emoji; `fill` bounds concurrent work at three and counts unavailable files.
- **Verified by running:** Android `:app:testDebugUnitTest :app:assembleDebug --offline` and `python scripts/verify.py --quick --android --offline` passed. The JVM tests cover catalogue exclusions/deduplication and a failed hash/size promotion leaving no durable target.
- **Not connected yet:** The per-device Keep everything setting, automatic post-connect fill, manual recheck/progress and space-used readout. No phone test for this slice; the updated APK was not installed.

#### V8 · Device setting and full-sync controls (2026-09-25)
- **Status: code complete, device audit pending.** Android defaults Keep everything on in its encrypted device store. A live socket schedules one background file fill five seconds after connecting; one shared fill lock prevents overlapping download sweeps. Settings shows the device-only toggle, Sync everything now, checked scope count, file count with unavailable count, and app storage used (files, cache and databases). The manual action sends core `recheck()` pulls and waits for all `caught` answers plus an empty `repairing()` list, with a two-minute timeout. If retention is on, it fills files after the recheck. The setting never queues an account op.
- **Verified by running:** Android Kotlin compilation and JVM tests, plus `python scripts/verify.py --quick --android --offline`, pass. No APK was installed on the phone; the owner unplugged it and asked that later phone runs be batched.
...
#### V10a · Re-upload local blobs after a server restore (2026-09-25)
- **Status: code complete, device audit pending.** After a `welcome` with `reconcile: true`, Android asks the core for `restoringBlobs()`, copies the locally available hashes into a durable, account-scoped `restore-blobs` queue after checking SHA-256 and the 100 MB upload limit, and starts `UploadWork`. Sources are pending uploads, retained files, and chat/avatar/emoji caches; it never fetches a missing file from the restored server. The worker sends normal and restore queues independently, resumes with `HEAD`, and treats 200 or 403 as already present, matching the web uploader. It deletes only the queued copy after success; original local copies remain. Missing local bytes are skipped.
- **Verified by running:** Android `:app:testDebugUnitTest :app:assembleDebug --offline` and `python scripts/verify.py --quick --android --offline` passed. A JVM test checks verified local-copy queueing, a bad hash and repeat queueing. No restore was simulated on the phone; the owner has unplugged it for now.

#### V10b · Android Sync issues (2026-09-25)
- **Status: code complete, device audit pending.** The sync client now reads core `syncIssues()` after restoration and each persisted frame and exposes it to Settings. Refused operations are listed oldest first with kind, server reason and local time. A refused message's retained text is visible and copyable; Dismiss calls core `dismissIssue()` and persists its removed op and metadata in the encrypted store. No server or core semantics changed. The core has no retry action for a permanently refused op; text can be copied into a new send.
- **Verified by running:** Android `:app:testDebugUnitTest :app:assembleDebug --offline` and `python scripts/verify.py --quick --android --offline` passed. No handset run; batch this with the next owner-approved phone audit.

#### V10c · Chat edit history (2026-09-25)
- **Status: code complete, device audit pending.** Android now reads the projection's edit count for chat messages, including older paged rows, and shows an `(edited)` control only when it is nonzero. Tapping it asks core `revisions(entity)` for the original and accepted edits from the local replica. The history stays behind the current message's content-warning reveal, and each historical version with its own warning has a separate Show/Hide gate. No server request or new data copy is needed. No edit composer was added in this slice.
- **Verified by running:** Android `:app:testDebugUnitTest :app:assembleDebug --offline` and `python scripts/verify.py --quick --android --offline` passed. A JVM test checks the projection edit count in both the normal and older-message windows. No phone test yet.

#### V10c · Default speaker per channel (D-074, 2026-09-25)
- **Status: code complete, device audit pending.** Android reads the account's `autoproxy:<channel>` pref with pending-local precedence and passes its mode/member, current front, self member, live members and latest own authors in that channel to the core `defaultSpeaker` rule. The last-own-authors lookup includes messages older than the newest 100 shown in Chat. The composer uses the core result until someone picks an author manually, and clears that pick on channel/account or default-setting changes. More → Advanced offers Front, Last speaker, Ask me and One member with a member chooser; changes queue account `pref.set` ops. A person account always follows its self member, per core. No server/core logic was reimplemented.
- **Verified by running:** Android `:app:testDebugUnitTest :app:assembleDebug --offline` and `python scripts/verify.py --quick --android --offline` passed. JVM tests cover pref precedence, the exact context passed to core and latch authors outside the display window. No phone test yet.

#### V10c · Shared message search on Android (2026-09-26)
- **Status: code complete, device audit pending.** Local message search now sends the whole search box to core `searchParse`/`searchFilter`, including `is:pinned`, `has:link`, dates and age filters; its candidate snapshot carries text, CW, raw member/channel names, attachment MIME types, links and pin state. Invalid filters show a message and do not request a server page. The server gets the same raw query and time-zone offset, including filter-only searches, with the existing cursor and local-ID deduplication. Posts and switches keep their existing local search. No core/server semantics changed.
- **Verified by running:** Android `:app:testDebugUnitTest :app:assembleDebug --offline` and `python scripts/verify.py --quick --android --offline` passed, including Rust `fixtures/search`. JVM tests check core candidate fields, result order, parser errors and raw request encoding. No handset or live search request was run; batch a phone check of pinned/link/filter-only searches with the next owner-approved device session.

#### V10c · Per-member reading on Android (2026-09-26)
- **Status: code complete, device audit pending.** Advanced Settings offers the account pref `chat.read_per_member`. While Chat is open, Android marks the latest visible message for the account and, when enabled, the fronting/co-conscious members chosen by core `readReaders`. It parses only this account's and pending local read marks, keeps the furthest per member, and skips marks already at or beyond the message. Core `readUnseenBy` computes the small unread dot on each message; tapping it names the members. The per-message work runs off the UI thread. No other account's read marks are displayed.
- **Verified by running:** Android `:app:testDebugUnitTest :app:assembleDebug --offline` and `python scripts/verify.py --quick --android --offline` passed. JVM tests cover account isolation, pending mark precedence, core contexts and pref precedence. No phone test yet. At the next batched session, toggle the setting, read a channel with two fronting members, then send a later message and check the dot/names.
