...
#### Journal read follow-up · server done, web pending
...
- The new real-HTTP integration test uses three accounts to check private/follower/bucket/server/deleted/malformed visibility, direct 404s, nested reply filtering, author/time/limit filters, author cards, no front snapshot, authentication, and access after a follow ends. It and server clippy passed. Run `verify.py --quick` before committing this piece.
- Audit first: cross-account posts still need a web reader, attachment access and reaction detail. Verify whether a visible quote snapshot needs additional redaction. This read API does not change `chorus_core` semantics or a settled decision.
