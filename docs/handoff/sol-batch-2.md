...
#### T7 · doing
- This first atomic piece adds account-authored JSONL op export and seven account-filtered tidy CSVs under `GET /api/v1/exports/...`, with an `export` API-token scope and downloads on the web Your data page. It reuses `api_data::principal`, so both device sessions and scoped API tokens resolve to one account. The HTTP integration test runs a real server, checks token permission and no authentication, checks all seven CSV routes, verifies RFC-style CSV quote escaping, and proves Alice's exports contain no Bob-only member or op payload. No core semantics changed.
- Next: implement the filtered SQLite copy, add CLI `export`, document CSV columns in DATA_MODEL, run the browser download and CLI export paths, then mark T7 done. Audit first: the current JSONL includes applied ops authored by the account across its scopes; check that this matches the intended "full" import contract when the other formats arrive.
